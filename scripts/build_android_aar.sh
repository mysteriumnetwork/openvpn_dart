...
# because grep -q closes the pipe on its first match.
...
for abi in arm64-v8a armeabi-v7a x86 x86_64; do
  ...
  for lib in libopenvpn.so libovpnexec.so; do
    grep -q "jni/${abi}/${lib}" <<<"$AAR_LIST" || { echo "ERROR: missing ${abi}/${lib}"; exit 1; }
  done
done
grep -q "ic_launcher" <<<"$AAR_LIST" && { echo "ERROR: launcher resources leaked into AAR"; exit 1; }
